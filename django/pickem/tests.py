...
class MissingCalculationTests(TestCase):
    def setUp(self):
        ...
        self.picks = []
        self.games = []
        self.season = Season.objects.create(year=2000, start_time=datetime.datetime(2000, 12, 26))
        for i in range(0, 12):
    ...
    """
    TODO: Delete these and views.select_all. Since the React move, I think this
    is dead code. But I'd like to more carefully confirm in an official delete
    pass
    """
    @skip("select_all view isn't used anymore? TODO: delete?")
    def test_missing_count_all_missing_solo(self):
        '''If no one has any picks, everyone is missing all games
        '''
        count = views.num_missing_picks_user(self.userA, self.season)
        self.assertEqual(count, len(self.games))
        count = views.num_missing_picks_user(self.userB, self.season)
        self.assertEqual(count, len(self.games))
    ...
    """
    TODO: Delete these and PrettyPicksView. Since the React move, I think this
    is dead code. But I'd like to more carefully confirm in an official delete
    pass
    """
    @skip("PrettyPicks isn't used anymore? TODO: delete?")
    def test_user_progresses_none(self):
        progress = views.PrettyPicksView.get_user_progresses(False, self.season)
...
class ScoreTableDBTests(TestCase):
    def setUp(self):
        self.num_games = 5
        self.users = (User.objects.create(username="bob"),
                      User.objects.create(username="alice"))
        self.season = Season.objects.create(year=2000, start_time=datetime.datetime(2000, 12, 26))
        self.events = [Event.objects.create(name=f"Game{i}") for i in range(self.num_games)]
        self.games = [Game.objects.create(
            event=self.events[i], datetime=timezone.now(), season=self.season)
                for i in range(self.num_games) ]
        self.teams = [ Team.objects.create(name=f"Team{i}") for i in range(
            len(self.games)*2) ]
    ...
    def test_all_unplayed_games(self):
        table = views.ScoreTable(self.users, self.season)
...
class ScoreTableTests(TestCase):
    def test_scores_sort_correct(self):
        season = Season.objects.create(year=2000, start_time=datetime.datetime(2000, 12, 26))
        table = views.ScoreTable([], season)
...
@patch("django.conf.settings.PICKEM_START_TIME", datetime.datetime(2023, 12, 26, 12, 0, 0))
class StarttimeTests(TestCase):
    def test_now_before_starttime_hasnt_started(self):
        reference_time = datetime.datetime(2023, 12, 26, 11, 59, 59)
        current_season = Mock(year=2023)
        self.assertFalse(views.pickem_started(reference_time, current_season))

    def test_now_at_starttime_has_started(self):
        reference_time = datetime.datetime(2023, 12, 26, 12, 0, 0)
        current_season = Mock(year=2023)
        self.assertTrue(views.pickem_started(reference_time, current_season))

    def test_prior_season_always_started(self):
        reference_time = datetime.datetime(2023, 12, 26, 11, 0, 0)
        prior_season = Mock(year=2000)
        self.assertTrue(views.pickem_started(reference_time, prior_season))
...
class GetLatestTest(TestCase):
    ...
    def test_get_incompleted_all(self):
        ...
        self.assertEqual(ordered_games,
                views.PrettyPicksView.get_incomplete_games([], self.season))

    def test_get_incompleted_none(self):
        self.assertEqual([],
                views.PrettyPicksView.get_incomplete_games(self.games, self.season))

    def test_get_complete_all(self):
        self.make_winners(range(len(self.games)))
        ordered_games = sorted(self.games, key=lambda x: x.datetime)
        self.assertEqual(ordered_games,
                views.PrettyPicksView.get_completed_games(self.season))
